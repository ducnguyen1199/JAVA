					...
					<form class="form" action="/Elearning_FE/SignUpHome" method="post"
						...
						<c:if test="${opt != 1}">
							<input type="text" pattern="[A-Za-z]{1,}"
								...
								value="${user.getTaiKhoan()}" name="username"
							...
							<c:if test="${wrongcp == true }">
								<p style="color: red; font-size: 12px; margin-top: 0">Your password and confirmation password do not match.</p>
							</c:if>
							<input type="text" pattern="[A-Za-z ]{1,}"
								title="Tên bạn có các kí tự đặc biệt" value="${user.getHoTen()}"
								name="fullname" placeholder="Full name" Required>
							<input type="text" pattern="[0-9]{10,13}"
								value="${user.getSoDienThoai()}" placeholder="Phone Number"
						...
						<p>
							Already have login and password? <a
								href="/Elearning_FE/home-login">Sign in now</a>
						</p>
					</form>
